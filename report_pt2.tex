...
Using four datasets corresponding to different operating voltage ranges, a first-order continuous-time nominal model $P(s)$ was identified using a least-squares approach on standardized frequency response data. The identified nominal plant is given by:
\begin{align}
...
This model captures the linearized dynamics associated with small perturbations to the pump. We are simulating a feedforward controller with two different transfer functions. The second is frequency-independent mapping from voltage to flow rate which was calculated using least squares on step response data. We found the following relationship.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\textwidth]{plots/linear_regression.pdf}
	\caption{Linear regression of steady-state flow rate versus input voltage from step response data. The slope of $2.437$ LPM/V provides the feedforward mapping used in the simulation.}
	\label{fig:regression}
\end{figure}

The slope of this regression gives us the relationship between voltage and flow rate which we found to be $2.437$ (LPM/V). This gives us our feedforward term in our simulation.
...
Model uncertainty arises from the variation in pump dynamics across different operating voltages (nonlinearities) and unmodeled high-frequency dynamics. We forced a model of our plant to be second order simulating these unknown frequency dynamics. This uncertainty was characterized by a multiplicative uncertainty weight $W_2(s)$.
...
The robust stability and performance of the controller are verified through frequency-domain analysis. Figure~\ref{fig:robust_stability} shows the robust Nyquist plot, which demonstrates that the closed-loop system remains stable for all plant variations within the uncertainty bound $W_2(s)$.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\textwidth]{plots/robust_nyquist.pdf}
	\caption{Robust Nyquist plot showing the closed-loop system remains stable for all off-nominal plants within the uncertainty bound.}
	\label{fig:robust_stability}
\end{figure}

The robust performance condition $\| |W_1 S| + |W_2 T| \|_\infty < 1$ is verified in Figure~\ref{fig:robust_perf}, which shows that the weighted sensitivity and complementary sensitivity functions satisfy the design requirements across all frequencies.

\begin{figure}[ht]
	\centering
	\subfigure[Robust Performance]{\label{fig:robust_perf}\includegraphics[width=0.48\textwidth]{plots/robust_performance.pdf}}
	\subfigure[Sensitivity and Complementary Sensitivity]{\label{fig:S_T}\includegraphics[width=0.48\textwidth]{plots/S_T_weights.pdf}}
	\caption{Robust performance analysis: (a) Verification that $\| |W_1 S| + |W_2 T| \|_\infty < 1$, and (b) Bode plots of weighted sensitivity $|W_1 S|$ and weighted complementary sensitivity $|W_2 T|$.}
